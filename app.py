...
async def check_card_with_retry(card, sites, proxies, max_retries=2, max_proxy_tries=None):
    ...
    if not proxies: return _make_result(card, 'Dead', 'No proxies configured')
    last_err = 'Unknown error'
    MAX_PROXY_TRIES = max_proxy_tries if max_proxy_tries is not None else max(max_retries, 4)
    for attempt in range(MAX_PROXY_TRIES):
        ...
        try:
            ...
            last_err = str(e)
            if _is_proxy_err(last_err) and attempt < MAX_PROXY_TRIES - 1:
        ...
        if res.status == CheckStatus.APPROVED:
            ...
            return _make_result(card, 'Approved', msg, price=res.amount or '-',
                                proxy=proxy_raw, site=shop_url)
        if res.status == CheckStatus.DECLINED:
            msg = str(res.error or res.status_code or 'DECLINED')
            return _make_result(card, 'Dead', msg, site=shop_url)
...
async def update_mass_progress(user_id, message_id, results, checked, last_res=None):
    ...
    latest = ""
    if last_res:
        se = "✅" if last_res['status']=='Charged' else ("🔥" if last_res['status']=='Approved' else "🚫")
        t  = round(time.time() - results.get('last_card_time', time.time()), 2)
        latest = (
            f"\n<b>{'─'*24}</b>\n"
            f"⚡ <b>Latest:</b>\n"
            f"{se} <tg-spoiler><code>{last_res['card']}</code></tg-spoiler>\n"
            f"✨ {last_res['message'][:60]} 💫 {t}s"
        )
    text = pe(
        f"<b>🔥 {fi('Shopify Mass Check')}</b>\n"
        f"<b>{SEP}</b>\n"
        f"📌 <b>{fi('Total Cards')}:</b> {results['total']}\n"
        f"☄️ <b>{fi('Checked')}:</b> {checked}\n"
        f"✅ <b>{fi('Approved')}:</b> {len(results['approved'])}\n"
        f"🃏 <b>{fi('Charged')}:</b> {len(results['charged'])}\n"
        f"⚠️ <b>{fi('3DS')}:</b> {len(results.get('tds',[]))}\n"
        f"<code>{bar}</code>"
        f"{latest}"
    )
...
async def send_final_results(user_id, results):
    ...
    for r in results.get('charged', []):
        record_log(user_id, cname, cusername, r.get('card',''), 'Charged',
                   r.get('message',''), r.get('site',''), r.get('gateway','Shopify Payments'), r.get('price','-'))
    for r in results.get('approved', []):
        record_log(user_id, cname, cusername, r.get('card',''), 'Approved',
                   r.get('message',''), r.get('site',''), r.get('gateway','Shopify Payments'), r.get('price','-'))
    summary = pe(
        f"<b>🔥 {fi('Mass Check Finished')}</b>\n"
        f"<b>{SEP}</b>\n"
        f"📌 <b>{fi('Total Cards')}:</b> {results['total']}\n"
        f"☄️ <b>{fi('Checked')}:</b> {results['total']}\n"
        f"✅ <b>{fi('Approved')}:</b> {ap_count}\n"
        f"🃏 <b>{fi('Charged')}:</b> {ch_count}\n"
        f"⚠️ <b>{fi('3DS')}:</b> {len(results.get('tds',[]))}\n"
        f"<code>{bar}</code>\n"
        f"⏱️ <b>{fi('Time')}:</b> {h}h {m}m {s}s\n"
        f"<b>{SEP}</b>\n"
    ...
    if results['charged']:
        ...
        await bot.send_file(user_id, "charged.txt", caption=pe(f"🥇 <b>Charged Hits</b>\n{DEV_LINE}"), parse_mode='html')
    ...
    if results['approved']:
        ...
        await bot.send_file(user_id, "approved.txt", caption=pe(f"✅ <b>Approved Hits</b>\n{DEV_LINE}"), parse_mode='html')
        try: os.remove("approved.txt")
        except: pass
    failed = [r for r in results['dead'] if r.get('status') == 'Error' or (r.get('message','').lower().startswith('step') and 'failed' in r.get('message','').lower())]
    if failed:
        ...
        await bot.send_file(user_id, "failed.txt", caption=pe(f"⚠️ <b>Failed / Errors</b>\n{DEV_LINE}"), parse_mode='html')
...
async def ran_command(event):
    ...
    text = pe(
        f"<b>🎲 {fi('Random Mass Check')}</b>\n<b>{SEP}</b>\n"
        f"🌐 <b>Pool:</b> {len(all_sites)} sites (random per card)\n"
        f"🧨 <b>{fi('Total Cards')}:</b> {len(cards)}\n⚡ <b>{fi('Checked')}:</b> 0\n"
        f"✅ <b>{fi('Approved')}:</b> 0\n🥇 <b>{fi('Charged')}:</b> 0\n⚠️ <b>{fi('3DS')}:</b> 0\n"
...
async def mass_check_cmd(event):
    ...
    if len(cards) > limit:
        cards = cards[:limit]
        await event.reply(pe(f"⚠️ <b>File trimmed to {limit} cards</b> (your {get_user_tier(uid)} plan limit)."), parse_mode='html')
    _tier_msh   = get_user_amount_tier(uid)
    _tlabel_msh = AMOUNT_TIERS.get(_tier_msh, ('Any',))[0]
    _fline_msh  = f"💰 <b>Filter:</b> {_tlabel_msh}\n" if _tier_msh != 'any' else ""
    text = pe(
        f"<b>🔥 {fi('Shopify Mass Check')}</b>\n<b>{SEP}</b>\n"
        f"{_fline_msh}"
        f"🧨 <b>{fi('Total Cards')}:</b> {len(cards)}\n⚡ <b>{fi('Checked')}:</b> 0\n"
...
async def add_site_command(event):
    ...
    if not new_site.startswith('http'): await event.reply(pe("❌ URL must start with http."), parse_mode='html'); return
    curr = load_sites()
    if new_site in curr: await event.reply(pe("⚠️ Site already exists."), parse_mode='html'); return
...
async def cb_start_check(event):
    ...
    _fline_cb = f"💰 <b>Filter:</b> {_rlabel_cb}\n" if _eff_tier_cb != 'any' else ""
    text = pe(
        f"<b>🔥 {fi('Shopify Mass Check')}</b>\n<b>{SEP}</b>\n"
        f"{_fline_cb}"
        f"🧨 <b>{fi('Total Cards')}:</b> {len(cards)}\n⚡ <b>{fi('Checked')}:</b> 0\n"
